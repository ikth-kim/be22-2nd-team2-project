...
public class AuthController {
  ...
  @PostMapping("/login")
  public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<TokenResponse>> login(
  ...
  @PostMapping("/refresh")
  public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<TokenResponse>> refreshToken(
  ...
  @PostMapping("/logout")
  public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Void>> logout(
      @Parameter(description = "쿠키에 저장된 Refresh Token (자동으로 전송됨)", required = false) @CookieValue(name = "refreshToken", required = false) String refreshToken) {
    if (refreshToken != null) {
    ...
    return ResponseEntity.ok()
        ...
        .body(com.team2.nextpage.common.response.ApiResponse.success());
  }

  /**
   * Access Token과 Refresh Token을 body와 쿠키에 담아 반환
   */
  private ResponseEntity<com.team2.nextpage.common.response.ApiResponse<TokenResponse>> buildTokenResponse(
      TokenResponse tokenResponse) {
    ResponseCookie cookie = createRefreshTokenCookie(tokenResponse.getRefreshToken());
    return ResponseEntity.ok()
        .header(HttpHeaders.SET_COOKIE, cookie.toString())
        .body(com.team2.nextpage.common.response.ApiResponse.success(tokenResponse));
  }
