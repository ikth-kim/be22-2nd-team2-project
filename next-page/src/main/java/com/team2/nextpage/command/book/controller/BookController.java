...
public class BookController {
    ...
    @PostMapping
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Long>> create(
        ...
        return ResponseEntity.ok(com.team2.nextpage.common.response.ApiResponse.success(bookId));
    }
    ...
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Long>> append(@PathVariable Long bookId,
            @RequestBody @Valid SentenceAppendRequest request) {
        Long writerId = SecurityUtil.getCurrentUserId();
        Long sentenceId = bookService.appendSentence(bookId, writerId, request);
        return ResponseEntity.ok(com.team2.nextpage.common.response.ApiResponse.success(sentenceId));
    }
    ...
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Void>> complete(@PathVariable Long bookId) {
        Long writerId = SecurityUtil.getCurrentUserId();
        bookService.completeBook(bookId, writerId);
        return ResponseEntity.ok(com.team2.nextpage.common.response.ApiResponse.success());
    }
    ...
    @PatchMapping("/{bookId}")
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Void>> updateBookTitle(
    ...
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Void>> updateSentence(
            @PathVariable Long bookId,
    ...
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Void>> deleteBook(@PathVariable Long bookId) {
        Long requesterId = SecurityUtil.getCurrentUserId();
     ...
     */
    @Operation(summary = "문장 삭제", description = "문장을 삭제합니다. 삭제 후 순서는 자동 조정됩니다. 작성자 또는 관리자만 가능합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공"),
            @ApiResponse(responseCode = "403", description = "권한 없음"),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 문장")
    ...
    public ResponseEntity<com.team2.nextpage.common.response.ApiResponse<Void>> deleteSentence(
            @PathVariable Long bookId,
            @PathVariable Long sentenceId) {
        Long requesterId = SecurityUtil.getCurrentUserId();
